...
class Solution:
    def coinChange(self, coins: List[int], amount: int) -> int:
        ...
        Int_Max = sys.maxsize
        dp = [[Int_Max - 1] * (amount + 1) for i in range(len(coins) + 1)] # placing max value as we want to find minimum
        ...
        for j in range(amount + 1):
            dp[0][j] = Int_Max - 1 # "-1" as if 1 is added then it does not cross Int_Max 
...
# *** IN THIS QUESTION WE HAVE TO INITIALIZE 1st ROW ALSO ***
        ...
        ans = dp[-1][-1] 
        return ans if ans != Int_Max - 1 else -1
